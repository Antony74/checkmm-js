...
(function (std) {
    ...
    var IStream = /** @class */ (function () {
        function IStream(filename) {
            this.buffer = Buffer.alloc(1);
            this.arr = [];
            this.fd = fs.openSync(filename, 'r');
        }
        IStream.prototype.get = function () {
            if (this.arr.length) {
                return this.arr.shift();
            }
            else {
                var bytesRead = fs.readSync(this.fd, this.buffer, 0, 1, null);
                return (bytesRead === 1) ? this.buffer.toString() : null;
            }
        };
        IStream.prototype.unget = function (s) {
            this.arr.unshift(s);
        };
        return IStream;
    }());
...
var CheckMM = /** @class */ (function (_super) {
    ...
    CheckMM.prototype.readtokens = function (filename) {
        ...
        var fin = new std.IStream(filename);
        var incomment = false;
    ...
    CheckMM.prototype.checkmm = function (filename) {
        this.setState({});
        var okay = this.readtokens(filename);
        if (!okay) {
            return false;
        }
...
function main(argv) {
    ...
    var okay = checkmm.checkmm(argv[1]);
